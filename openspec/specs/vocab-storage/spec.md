...
The system SHALL save a word only when the user is authenticated. When the user clicks "Save" in the definition popup — a control shown only while signed in — the system SHALL write the entry to `chrome.storage.local` and additionally upsert the word to Supabase `vocab_entries`, appending the new encounter to the existing `encounters` array on conflict. Each save SHALL record the word's definition snapshot, the source article URL, and a timestamp. The encounter's `sentence` field SHALL be optional and SHALL NOT capture the surrounding page text; saves from the content script SHALL record an empty `sentence`. When the user is not authenticated, the definition popup SHALL NOT offer "Save"; it SHALL surface a "Sign in with Google" affordance instead, and no entry SHALL be written.
...
- **THEN** the encounter SHALL record the source URL and timestamp with an empty `sentence`
